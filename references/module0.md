...
是可以解释的**，不是玄学。**历史示例**（下面这三个数是早期版本的，
现在对不上了 —— 保留是为了说明"版本差异长什么样"）：
...
> **这张表已经过时，别拿它当基准。** 现在的权威值在 CI 日志与
> `domain_status.json` 里：不平滑 11 域 / 邻居同域率 `0.507`（基线
> `0.111`），平滑后 13 域 / `0.668`，Moran's I（空间平滑后）`0.7670`。
> **文档里写死数值就会过时** —— 这正是"结论要跟着产物走"的理由
> （§9.1 末尾、以及各状态 JSON 的 `reproducibility` 字段）。

**反过来也要注意：** 跨轮一致的量（域数 13、Moran's I 0.7670、
通讯 62/63 对可用）才是可以当结论用的。报数时区分这两类。
