...
def _record_figure_overflow(cfg: dict, name: str, bad: dict) -> None:
    ...
    Q-26 / E-49 的教训（本仓 Q-27 同步）：`_content_overflow()` **早就
    检测到了** `{'width_overflow_frac': 0.3523}`、也**打了 WARN**，而那张图的
    标题两侧仍被静默裁掉 35% —— 因为**告警没有消费者**。
    "人读 CI 日志"已被证明会漏。

    所以判据强度要从**警告级**升到**产物级**：写进 `figure_overflow.json`，
    由 `main_analysis.py` 的验收项读它并判红。文件写在 `figures_dir` 的
    上一级（即 `results/<dataset>/`），与其余 status/产物同级。
    """
