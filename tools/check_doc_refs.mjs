...
// ---------------------------------------------------------------------------
// 在提到该文件的**同一行、上一行或下一行**写出下面任一标记即可：
//
//   1. 文件已删：  已删 / 已移除 / 不再存在 / 曾经 / 当时的 / 旧名 / 原名 /
//                  deleted / removed
...
const DELETED_MARK = /已删|已移除|不再存在|曾经|当时的|旧名|原名|deleted|removed/i
...
if (nExempt) console.log(`  逃生舱（同/邻行标了"已删"或"第三方包"）  ${nExempt} 条豁免`)
...
三种改法，三选一：
  ...
  2. 那个文件**确实已经删掉** → 在它的同一行、上一行或下一行写明
     "已删 / 已移除 / 不再存在 / 曾经 / 当时的 / deleted"。
     **只留结论，不留死链接。**
